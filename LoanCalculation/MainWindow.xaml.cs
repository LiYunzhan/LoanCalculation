...
namespace LoanCalculation
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void button_Copy_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (config == null)
            {
                System.Windows.MessageBox.Show("seting.json文件格式有问题", "提示");
                return;
            }
            foreach (var item in config)
            {
                item.EndDate = DateTime.ParseExact(item.EndDateStr, "yyyy-MM-dd", System.Globalization.CultureInfo.CurrentCulture);
                item.StartDate = DateTime.ParseExact(item.StartDateStr, "yyyy-MM-dd", System.Globalization.CultureInfo.CurrentCulture);
		...
		/// <returns></returns>
		public static List<Loan> ReadJson(string cfgFilePath)
        {
            List<Loan> cfgModel = new List<Loan> { };
            //序列化  
            string cfgSerializer = JsonConvert.SerializeObject(cfgModel);

            //反序列化  
            List<Loan> debc1 = JsonConvert.DeserializeObject<List<Loan>>(cfgSerializer);
            cfgFilePath = "./setting.json";
            //读取json文件  
            using (StreamReader sr = new StreamReader(cfgFilePath))
            {
                try
                {
                    ...
                    cfgModel = serializer.Deserialize<List<Loan>>(reader);
                    return cfgModel;
                }
                catch (Exception ex)
                {
